 ...
 */
export async function deduplicateAllUserWallets(): Promise<void> {
  try {
    ...
    if (Array.isArray(duplicates) && duplicates.length > 0) {
      ...
      for (const group of duplicates) {
        ...
        for (const w of wallets) {
          ...
          if (!bestAddress && w.address) bestAddress = w.address;
        ...
        if (toDelete.length > 0) {
          await Wallet.deleteMany({ _id: { $in: toDelete } });
        }
    ...
    await Wallet.syncIndexes().catch((idxErr: any) => {
